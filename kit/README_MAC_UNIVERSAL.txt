...
Church / guest session: download ChurchGuestZoom-20260823N.zip.
...
3. Click Continue if a dialog is waiting (it continues on its own after a few seconds).
   A Mac password box may appear behind other windows.
...
WHY EARLIER BUILDS DID NOTHING / FELT STUCK
...
were still on disk, so Zoom never opened. Build M still starts Zoom but
waited on leftover parks, extra dialogs, and system_profiler (slow on a
2015 Air). Build N opens Zoom first and restores leftovers after you quit.
