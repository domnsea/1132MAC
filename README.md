...
Download **`ChurchGuestZoom-20260823L.zip`** only:

https://github.com/domnsea/1132MAC/raw/cursor/fix-zoom-launch-crash-8c90/ChurchGuestZoom-20260823L.zip

Unzip. **Right-click** `ChurchGuestZoom-OPEN-ME.command` or `ChurchGuestZoom.app` → **Open**.

Throw away zip **K**. It started Zoom with `sandbox-exec`, so Zoom showed **no microphones** (VB-Cable and the built-in mic both missing).
